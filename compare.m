%% quantify the similarity between ground-truth image volumes and deniosed image volumes
clear, clc, close all
...
dpPred = fullfile(dpRoot, 'pred-denoise', 'hcp001_denoise.mat');
...
[t1w_lowsnr_norm, t1w_norm] = norm(t1w_lowsnr, t1w, mask);
[t1w_lowsnr_norm, pred_norm] = norm(t1w_lowsnr, pred, mask);

t1w_lowsnr_diff = t1w_lowsnr_norm - t1w_norm;
pred_diff = pred_norm - t1w_norm;

%% MAD

t1w_lowsnr_mad = mean(abs(t1w_lowsnr_norm - t1w_norm),'all'); 
pred_mad = mean(abs(pred_norm - t1w_norm),'all'); 

%% PSNR

t1w_lowsnr_psnr = psnr(t1w_lowsnr_norm, t1w_norm); 
pred_psnr = psnr(pred_norm, t1w_norm);

%% SSIM

[t1w_lowsnr_ssim, t1w_lowsnr_ssimmap] = ssim(t1w_lowsnr_norm,t1w_norm);
