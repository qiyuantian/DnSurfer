...
# %% set up path
dpRoot = os.path.dirname(os.path.abspath('s_DnSurfer_train.py'))
os.chdir(dpRoot)
...
for ii in np.arange(1, len(files)): # start from 1, use 0th subject for evaluation
    ...
    img_in, img_out = utils.normalize_image(t1w_lowsnr, t1w, mask) 
...
nin = 1
nout = 1
dtnet = dnsurfer_3d_model(nin, nout, filter_num=nfilter)
dtnet.summary()
