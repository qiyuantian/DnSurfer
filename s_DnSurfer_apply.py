...
from dnsurfer import dnsurfer_3d_model
...
# %% set up path
dpRoot = os.path.dirname(os.path.abspath('s_DnSurfer_apply.py'))
os.chdir(dpRoot)
...
# %% load model
dpCnn = os.path.join(dpRoot, 'dnsurfer')
fnCp = 'dnsurfer_ep30'
...
for ii in np.arange(0, 1):  # start from 1, use 0th subject for evaluation
    ...
    t1w_lowsnr = tmp['t1w_lowsnr']

    t1w_lowsnr = np.expand_dims(t1w_lowsnr, 3)
    t1w = np.expand_dims(t1w, 3)
    mask = np.expand_dims(mask, 3)

    # %% standardize image intensities
    img_in, tmp = utils.normalize_image(t1w_lowsnr, t1w_lowsnr, mask)
    img_tiw, tmp = utils.normalize_image(t1w, t1w, mask)
    ...
    img_pred_denorm = utils.denormalize_image(t1w_lowsnr, img_pred, mask)  # transform to normal intensity range

    dpPred = os.path.join(dpRoot, 'pred-denoise')
    if not os.path.exists(dpPred):
        os.mkdir(dpPred)
        print('create directory')

    fpPred = os.path.join(dpPred, fnData[0:6] + '_denoise.mat')
   ...
   # plt.savefig(dpRoot + '/dnsurfer_ep30.png')
    plt.show()
